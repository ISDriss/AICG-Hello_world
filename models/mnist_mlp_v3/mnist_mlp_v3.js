...
const r_32_4_4_8_64n1 = `fn nan() -> f32 { let bits = 0xffffffffu; return bitcast<f32>(bits); }
@group(0) @binding(0)
var<uniform> INFINITY : f32;
var<workgroup> temp0: array<f32,128>;
@group(0) @binding(1)var<storage,read_write>data0_512:array<f32>;
...
const setupNet = async (device, safetensor) => {
    ...
    const buf_6 = createWeightBuf(device, 1048576, getTensorBuffer(safetensor, metadata['layers.5.weight']));
    const buf_7 = createWeightBuf(device, 2048, getTensorBuffer(safetensor, metadata['layers.5.bias']));
    const output0 = createEmptyBuf(device, 40);;
    const buf_8 = createWeightBuf(device, 20480, getTensorBuffer(safetensor, metadata['layers.7.weight']));
    const buf_9 = createWeightBuf(device, 40, getTensorBuffer(safetensor, metadata['layers.7.bias']));

    const gpuWriteBuffer0 = device.createBuffer({size:input0.size, usage: GPUBufferUsage.COPY_SRC | GPUBufferUsage.MAP_WRITE });

    const gpuReadBuffer0 = device.createBuffer({size:output0.size, usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ });

    const kernels = [r_32_4_4_8_98, r_32_4_4_8_64, r_32_4_4_8_64n1, r_10_16_32];
    ...
    return async (_input0) => {
        ...
        addComputePass(device, commandEncoder, pipelines[2], layouts[2], infinityBuf, [buf_0, buf_3, buf_6, buf_7], [32, 1, 1]);
        addComputePass(device, commandEncoder, pipelines[3], layouts[3], infinityBuf, [output0, buf_0, buf_8, buf_9], [10, 1, 1]);
